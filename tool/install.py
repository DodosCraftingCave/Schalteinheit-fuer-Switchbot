...
# damit Installer und Tool wie aus einem Guss wirken.
BG            = "#000000"  # --bg
SURFACE       = "#0A0A0B"  # --surface
SURFACE_ALT   = "#161617"  # --surface-alt
BORDER        = "#2B3542"  # --border
TEXT          = "#EDF1F5"  # --text
MUTED         = "#8996A6"  # --muted
ACCENT        = "#4C8DFF"  # --accent
ACCENT_STRONG = "#3B72D6"  # --accent-strong (hover)
ACCENT_INK    = "#FFFFFF"  # --accent-ink
SUCCESS       = "#3ECF8E"  # --success
DANGER        = "#FF6B6B"  # --danger — auch die Headline-Farbe im Tool
...
class InstallerApp(tk.Tk):
    ...
    def _build(self):
        ...
        tk.Label(hdr, text="SwitchBot Konfigurator",
                 font=("Segoe UI",16,"bold"), bg=SURFACE, fg=DANGER).pack()
        tk.Label(hdr, text="Installation", font=("Segoe UI",10),
                 bg=SURFACE, fg=MUTED).pack()

        info = tk.Frame(self, bg=SURFACE_ALT, pady=12)
        ...
        style.configure("accent.Horizontal.TProgressbar",
                        troughcolor=SURFACE_ALT, background=ACCENT,
                        bordercolor=SURFACE_ALT, lightcolor=ACCENT, darkcolor=ACCENT)
        ...
        self.install_btn = tk.Button(btn_frm, text="🚀  Jetzt installieren",
            command=self._start, bg=ACCENT, fg=ACCENT_INK,
            activebackground=ACCENT_STRONG, font=("Segoe UI",11,"bold"),
            relief="flat", cursor="hand2", pady=9)
        self.install_btn.pack(fill="x")

        self.close_btn = tk.Button(btn_frm, text="Schließen",
            command=self.destroy, bg=SURFACE_ALT, fg=MUTED,
